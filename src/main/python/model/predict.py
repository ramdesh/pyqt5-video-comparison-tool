...
import model.model_modified as md
...
def read_images(image_path):
    print("Attempting to read images from " + image_path)
...
def contour_func(nerve_mask, original_image, ids, nerve, shape):
    ...
    contour_marked = cv2.drawContours(original_image, [contours[0]], contourIdx=-1, color=(0, 255, 0), thickness=2)
...
def predict():
    cur_dir = os.getcwd()
    to_predict, ids = read_images(image_path=cur_dir + '/images/frames/')
    asm_masks = predict_masks(images_npy=to_predict, model=unet_model, h5_path=cur_dir + '/h5_files/unet-tgs-weights_100original_asm.h5')
    bp_masks = predict_masks(images_npy=to_predict, model=unet_model, h5_path=cur_dir + '/h5_files/unet-tgs-weights_100original_bp.h5')
    msm_masks = predict_masks(images_npy=to_predict, model=unet_model, h5_path=cur_dir + '/h5_files/unet-tgs-weights_100original_msm.h5')
    scm_masks = predict_masks(images_npy=to_predict, model=unet_model, h5_path=cur_dir + '/h5_files/unet-tgs-weights_100original_scm.h5')
    mask_to_image('asm', asm_masks, cur_dir + '/images/mask_predicted', ids)
    mask_to_image('bp', bp_masks, cur_dir + '/images/mask_predicted', ids)
    mask_to_image('msm', msm_masks, cur_dir + '/images/mask_predicted', ids)
    mask_to_image('scm', scm_masks, cur_dir + '/images/mask_predicted', ids)
    draw_contours('asm', cur_dir + '/images/mask_predicted/', cur_dir + '/images/frames/', cur_dir + '/images/predicted/', ids)